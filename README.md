...
### v0.1.4
- ✅ 修复任务栏图标显示问题：使用 Windows API (WM_SETICON) 在窗口显示后强制设置图标，兼容控制台和 GUI 两种打包模式
- 🖱️ 新增双击跳转功能：扩展列表、对话记录、工作区列表、MCP路径均可双击打开对应文件夹
- 📝 设置页面新增操作提示说明（悬停提示和双击跳转）
- ⚡ 优化悬浮提示响应速度和灵敏度，改为纯文本显示
- 📊 新增全局存储文件分析能力：支持查看大文件夹内文件大小分布，定位占用大的文件，双击可打开对应位置
- 🔧 配置文件存储于用户目录 `.trae-cache-cleaner` 隐藏文件夹，包含程序设置、扫描缓存（加速启动）等
- 📌 版本号规范修正
- 📱 设置窗口优化：导入导出路径字号调大，窗口高度增加
- 🗂️ MCP表格优化：启用栏自动收缩，路径栏支持双击跳转
- 🚀 响应速度、内存占用优化与代码精简
- 🔲 启动GUI后控制台窗口自动最小化（窗口显示后再最小化，避免用户看不到GUI）
- 🔄 新增静默自动刷新：当用户通过路径双击打开文件夹后，自动检测文件变化并刷新列表
- 🐛 修复配置导出/导入显示问题（缓存加载后正确加载设置项）
- 🐛 修复分析全局存储弹窗一闪而过问题（保存为实例变量防止被垃圾回收）
