...
class PathConfig:
    ...
    @staticmethod
    def get_config_dir():
        """获取配置文件目录（用户目录下的隐藏文件夹）"""
        config_dir = os.path.join(os.path.expanduser('~'), '.trae-cache-cleaner')
...
if HAS_PYQT5:
    ...
    class MainWindow(QMainWindow):
        ...
        def init_chat_tab(self):
            ...
            self.chat_list.doubleClicked.connect(self.on_chat_double_clicked)
        ...
        def init_workspace_tab(self):
            ...
            self.workspace_list.doubleClicked.connect(self.on_workspace_double_clicked)
        ...
        def init_extension_tab(self):
            ...
            self.extension_table.doubleClicked.connect(self.on_extension_double_clicked)
        ...
        def init_settings_tab(self):
            ...
            g2_layout.addWidget(hint_group)
            ...
            about_text = QLabel(
                f"<b>TraeCacheCleaner</b> {VERSION}<br><br>"
                "Trae IDE 缓存清理工具<br>"
        ...
        def on_extension_double_clicked(self, index):
            """双击扩展表格打开安装目录"""
            row = index.row()
            if row < len(self.extension_items):
        ...
        def on_chat_double_clicked(self, index):
            """双击对话记录打开关联文件所在目录"""
            row = index.row()
            if row < len(self.chat_items):
                path = self.chat_items[row].get('path', '')
                if path:
                    # 获取目录路径
        ...
        def on_workspace_double_clicked(self, index):
            """双击工作区项目打开对应目录"""
            row = index.row()
            if row < len(self.workspace_items):
